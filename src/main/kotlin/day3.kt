...
private fun String?.hasSymbolIn(range: IntRange): Boolean = this?.substring(range)?.any { it in SYMBOLS } ?: false

fun day3part1(): Int {
  val lines = input.lines()
  var sum = 0

  for ((lineNumber, currentLine) in lines.withIndex()) {
    val previousLine = lines.getOrNull(lineNumber - 1)
    val nextLine = lines.getOrNull(lineNumber + 1)
    val numberMatches = numberRegex.findAll(currentLine)

    for (match in numberMatches) {
      val range = (match.range.first - 1).coerceAtLeast(0)..(match.range.last + 1).coerceAtMost(currentLine.length - 1)
      if (previousLine.hasSymbolIn(range) || currentLine.hasSymbolIn(range) || nextLine.hasSymbolIn(range)) {
...
fun day3part2(): Int {
  ...
  for ((lineNumber, currentLine) in lines.withIndex()) {
    val previousLine = lines.getOrNull(lineNumber - 1)
    val nextLine = lines.getOrNull(lineNumber + 1)
    val potentialGearIndices = currentLine.mapIndexedNotNull { index, char -> index.takeIf { char == '*' } }

    for (index in potentialGearIndices) {
      val numbers = currentLine.getNumbersAround(index) +
        previousLine.getNumbersAround(index) +
        nextLine.getNumbersAround(index)
...
private fun String?.getNumbersAround(index: Int): List<Int> {
  if (this == null) return emptyList()
  val range = (index - 1).coerceAtLeast(0)..(index + 1).coerceAtMost(length - 1)
  return numberRegex.findAll(this)
    .filter { range.intersect(it.range).isNotEmpty() }
    .map { it.value.toInt() }
    .toList()
}
