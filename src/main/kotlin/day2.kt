...
private val countsRegex = """(?<count>\d+) (?<colour>[\w\s]+)""".toRegex()

fun day2part1(): Int {
  return input.lineSequence()
    .mapIndexed { index, line ->
      val (red, green, blue) = countCubes(line)
      if (red <= MAX_RED && blue <= MAX_BLUE && green <= MAX_GREEN) index + 1 else 0
    }
    .sum()
}

private data class HighestCubeCounts(val red: Int, val green: Int, val blue: Int)

private fun countCubes(game: String): HighestCubeCounts {
  val colorCounts = mutableMapOf<String, Int>()

  for (result in countsRegex.findAll(game)) {
    val (countString, color) = result.destructured
  ...
  return HighestCubeCounts(
    red = colorCounts.getOrDefault("red", 0),
    green = colorCounts.getOrDefault("green", 0),
    blue = colorCounts.getOrDefault("blue", 0),
  )
}
