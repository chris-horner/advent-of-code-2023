fun day1part1(): Int = input.lines().sumOf { line ->
  val first = line.first { char -> char.isDigit() }
  val last = line.last { char -> char.isDigit() }
  "$first$last".toInt()
}

private val wordMap = listOf("one", "two", "three", "four", "five", "six", "seven", "eight", "nine")
  .mapIndexed { index, word -> word to (index + 1).toString() }.toMap()
private val regex = """(?=(\d|one|two|three|four|five|six|seven|eight|nine))""".toRegex()
private fun String.toNumber(): String = if (length == 1) this else wordMap[this]!!

fun day1part2(): Int = input.lines().sumOf { line ->
  val matches = regex.findAll(line).map { it.groupValues.last() } // Regex lookahead puts value in last group.
